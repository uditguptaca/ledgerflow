    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Track and pay supplier invoices and accounts payable.</p>
        </div>
        <Link
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-soft">
        <div className="overflow-x-auto">
          {loading ? (
            <div className="text-center py-12 text-sm text-slate-500">Loading bills...</div>
          ) : (
                ...
                {filteredBills.map((b) => {
                  ...
                  return (
                    <tr key={b.id} className="hover:bg-slate-50/40 transition-colors">
                      ...
                      <td className="table-cell font-medium text-slate-800">{b.vendor?.name || 'Unknown'}</td>
                      <td className="table-cell font-mono text-xs">{b.date ? new Date(b.date).toISOString().split('T')[0] : '-'}</td>
                      <td className="table-cell font-mono text-xs">{b.dueDate ? new Date(b.dueDate).toISOString().split('T')[0] : '-'}</td>
                      <td className="table-cell text-right font-mono font-semibold text-slate-800">
                        ${Number(b.total).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </td>
