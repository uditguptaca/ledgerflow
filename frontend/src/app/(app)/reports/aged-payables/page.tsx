...
export default function AgedPayablesPage() {
  ...
  useEffect(() => {
    const fetchAgedPayables = async () => {
      try {
        setLoading(true);
        const res = await api.get<any>(`/v1/reports/aged-payables?asOfDate=${asOfDate}T23:59:59Z`);
        const mapped = (res.vendors || []).map((v: any) => ({
          vendor: v.vendorName,
          current: Number(v.current || 0),
          days1to30: Number(v.days1to30 || 0),
          days31to60: Number(v.days31to60 || 0),
          days61to90: Number(v.days61to90 || 0),
          days90plus: Number(v.days90plus || 0),
  ...
  const columns: ColumnConfig<AgedPayablesRow>[] = [
    {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">Aging Schedule: monitor vendor invoices due parameters.</p>
          </div>
        </div>
      ...
      {/* Report Table */}
      {loading ? (
        <div className="text-center py-12 text-slate-500">Loading Aged Payables...</div>
      ) : (
        <ReportTable
          data={data}
          columns={columns}
